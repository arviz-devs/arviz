...
The terminology used in this specification is based on {ref}`xarray's terminology <xarray:terminology>`. Below is a summary of the most relevant terms for this specification:
...
* Dimension names `chain`, `draw`, `sample` and `pred_id` are reserved for use to indicate sample dimensions.
  ...
  - `draw` indicates the iteration _within_ each MCMC chain.
    ArviZ assumes all chains have the same length for better interoperability with NumPy and xarray.
...
Both DataTree groups and variables can have metadata, which in the `unconstrained_posterior`
...
The same rules and conventions defined in {ref}`schema/sample_stats` apply to any sample stats group.
...
It is described in more detail in {ref}`schema/unconstrained_posterior` section, which is what we expect to be the most common section, but other groups could also have an unconstrained linked group, e.g. `prior` and `unconstrained_prior`.
